...
- **熔断保护** — 渠道级（按 API Key）+ 模型级两层熔断，自动隔离故障渠道，支持可配置重试次数与熔断时长、熔断状态展示与手动恢复
- **多模态规则** — 通过正则表达式匹配模型名称，自动标记模型支持的输入类型（text / image / audio / video），支持视频与音频内容的路由判断
- **模型继承** — 入口模型可继承另一个入口模型的渠道关联关系，减少重复配置
- **提示注入** — 按模型配置提示注入规则（注入角色、位置、内容、优先级），请求预处理时自动注入
- **API Key 管理** — 网关层面的密钥管控，支持启用/禁用、分享链接生成与回收
- **请求日志** — 全链路请求追踪，支持重试链路缩进展示、SSE 实时推送、原始请求数据按需加载与 JSON 文件下载
- **聊天界面** — 内置聊天对话界面，支持图片上传与多模态输入展示，支持 OpenAI / Anthropic 协议直连与思考块（thinking）多轮传递
- **模型测试** — Playground 在线调试模型响应，支持流式输出、多模态输入、首字节响应时间与准确 Token 统计
...
│       ├── entity/                   # 数据实体（12 个实体：渠道、模型、密钥、日志、熔断、多模态规则、提示注入等）
...
│   ├── src/
│   │   ├── api/                      # HTTP 请求封装（12 个模块：渠道、模型、密钥、日志、认证、仪表盘、聊天、分享、多模态、提示注入、系统、上传）
