      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <select v-model="selectedModel" class="form-control">
            ...
            <option v-for="m in models" :key="m.id" :value="m.modelName">{{ m.modelName }}</option>
          </select>
        </div>
...
async function sendMessage() {
  ...
  try {
    if (streamEnabled.value) {
      await sendStreamRequest(content, assistantMsg)
    } else {
      await sendNonStreamRequest(content, assistantMsg)
    }
  } catch (e: any) {
...
async function sendStreamRequest(content: string, targetMsg: ChatMessage) {
  const body = {
...
async function sendNonStreamRequest(content: string, targetMsg: ChatMessage) {
  const body = {
    model: selectedModel.value,
    messages: messages.value.filter(m => m.role !== 'system-msg' && m.content.trim() !== '').map(m => ({
      role: m.role === 'user' ? 'user' : 'assistant',
